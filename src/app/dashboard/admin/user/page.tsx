...
export default function UserEditorPage() {
  ...
  const [newUser,    setNewUser]    = useState<Users>({ ...BLANK, information: { ...BLANK.information } });
  ...
  function loadDataList() {
    fetch("/api/admin/", {
      method: "post",
      body: JSON.stringify({ method: "GET_USER" }),
    })
      .then(r => r.json())
  ...
  async function saveNew() {
    ...
    const res  = await fetch("/api/admin/", {
      method: "POST",
      body: JSON.stringify({ method: "ADD_USER", data: newUser }),
    });
    const json = await res.json();
    setProcess(false);
    if (json.status !== "FAIL") {
      setNewUser({ ...BLANK, information: { ...BLANK.information } });
      loadDataList();
      closeNew();
      showToast("User berhasil ditambahkan.");
    } else {
      showToast("Gagal menambahkan user.", false);
    }
  }
  ...
  return (
    <>
      ...
      <div className="io-overlay" ref={overlayNew} onClick={e => { if (e.target === overlayNew.current) closeNew(); }}>
        <div className="io-modal" onClick={e => e.stopPropagation()}>
          <div className="io-mhead">
            <span className="io-mtitle">Tambah User Baru</span>
          ...
          <div className="io-mbody">
            ...
            <div className="io-field">
              ...
              <input className="io-input" type="text" placeholder="cth. Budi Santoso"
                ...
                onChange={e => setNewUser(p => ({ ...p, information: { ...p.information, fullname: e.target.value } }))} />
            </div>
            <div className="io-field-row">
              <div className="io-field">
                ...
                <input className="io-input" type="email" placeholder="email@domain.id"
                  ...
                  onChange={e => setNewUser(p => ({ ...p, information: { ...p.information, email: e.target.value } }))} />
              </div>
              <div className="io-field">
                ...
                <input className="io-input" type="text" placeholder="08xx-xxxx-xxxx"
                  ...
                  onChange={e => setNewUser(p => ({ ...p, information: { ...p.information, phone: e.target.value } }))} />
              </div>
            </div>
          </div>
      ...
      <div className="io-overlay" ref={overlayEdit} onClick={e => { if (e.target === overlayEdit.current) closeEdit(); }}>
        <div className="io-modal" onClick={e => e.stopPropagation()}>
          <div className="io-mhead">
            <span className="io-mtitle">Detail User</span>
      ...
      <div className="io-overlay" ref={overlayDel} onClick={e => { if (e.target === overlayDel.current) closeDel(); }}>
        <div className="io-confirm-box" onClick={e => e.stopPropagation()}>
          ...
          <p className="io-confirm-title">Hapus user?</p>
          <p className="io-confirm-msg">
            User <strong>{deleteTgt?.name}</strong> akan dihapus permanen.
